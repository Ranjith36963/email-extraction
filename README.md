...
An AI-powered workflow that automatically extracts customer details, dimensions, requirements, and use cases from inbound B2B enquiry emails — then presents everything in a live dashboard.
...
```
                          AI Enquiry Triage — Processing Pipeline
...
1. **Email arrives** — A customer sends an enquiry to the configured inbox
2. **n8n triggers** — The automation platform detects the new email and checks for attachments
3. **GPT-4o extracts** — The email body (and any attached images via Vision API) is processed to extract structured data: customer info, product types, dimensions, quantities, requirements, urgency, and use case
